...
class NuscenesDD3DTargetPreparer(DD3DTargetPreparer):
    ...
    def __call__(self, locations, gt_instances, feature_shapes):
        ...
        attributes = cat([x.reshape(-1)
                         for x in training_targets["attributes"]])
...
class NuscenesLoss(nn.Module):
    ...
    def forward(self, attr_logits, speeds, fcos2d_info, targets):
        ...
        valid_attr_mask = target_attr != MAX_NUM_ATTRIBUTES

        if pos_inds.numel() == 0:
            attr_weights = attr_logits.new_tensor(
                0.0)  # torch.tensor(0.0).cuda()
        else:
        ...
        # NOTE: compute attr_weights_sum, and then feed it to reduce_sum()
        # works, but not above.
        attr_weights_sum = attr_weights.sum()
        attr_loss_denom = max(
            reduce_sum(attr_weights_sum).item() /
            d2_comm.get_world_size(),
            1e-6)
        ...
        speed_loss_denom = max(
            reduce_sum(speed_weights_sum).item() /
            d2_comm.get_world_size(),
            1e-6)
        ...
        if pos_inds.numel() == 0:
            losses = {
                "loss_attr": attr_logits.sum() * 0.,
                "loss_speed": speeds.sum() * 0.}
            # NOTE: This is probably un-reachable, because the training filter images with empty annotations.
            # NOTE: If not, attr_weights can be unavailable in the reduce_sum
            # below().
            return losses
...
class NuscenesInference():
    ...
    def __call__(self, attr_logits, speeds, pred_instances, fcos2d_info):
        ...
        for lvl, (attr_logits_lvl, speed_lvl, info_lvl, instances_lvl) in \
            ...
            attr_logits_lvl = attr_logits_lvl.permute(
                0, 2, 3, 1).reshape(
            ...
            for i in range(N):
                ...
                if len(attr_logits_per_im) == 0:
                    ...
                    instances_lvl[i].pred_attributes = attr_logits_per_im.argmax(
                        dim=1)
...
class NuscenesDD3D(DD3D):
    ...
    def forward(self, features, batched_inputs):
        ...
        if 'inv_intrinsics' in batched_inputs[0]:
            inv_intrinsics = [
                x['inv_intrinsics'].to(
                    features[0].device) for x in batched_inputs]
        ...
        logits, box2d_reg, centerness, fcos2d_extra_output = self.fcos2d_head(
            features)
        ...
        if self.training:
            ...
            if not self.only_box2d:
                # This adds 'pred_boxes3d' and 'scores_3d' to Instances in
                # 'pred_instances'.
            ...
            # This adds 'pred_attributes', 'pred_speed' to Instances in
            # 'pred_instances'.
            self.nuscenes_inference(
                attr_logits, speeds, pred_instances, fcos2d_info)

            # Transpose to "image-first", i.e. (B, L)
            pred_instances = list(zip(*pred_instances))
            pred_instances = [Instances.cat(instances)
            ...
            if not self.only_box2d and self.do_bev_nms:
                # Bird-eye-view NMS.
                dummy_group_idxs = {i: [i]
                                    for i, _ in enumerate(pred_instances)}
            ...
            if self.postprocess_in_inference:
                ...
                group_idxs = get_group_idxs(
                    sample_tokens, self.num_images_per_sample)
