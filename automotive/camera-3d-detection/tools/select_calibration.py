...
def read_pkl_and_select_random_items(file_path, num_items):
    ...
    selected_items = random.sample(filtered_data, min(num_items, len(filtered_data)))
...
def filter_scene_length(input_dict, min_value):
    return {key: value for key, value in input_dict.items() if value[1] >= min_value}
...
def main():
    parser = argparse.ArgumentParser(description="Select items from pickle files.")
    parser.add_argument("--scene_lengths", type=str, help="Path to the scene lengths pickle file")
    parser.add_argument("--scene_starts", type=str, help="Path to the scene starts pickle file")
    parser.add_argument("--num_items", type=int, default=10, help="Number of scenes to select")
