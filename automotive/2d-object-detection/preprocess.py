...
class Process():
    def __init__(self, dataset_root, image_size, files, transformer, label_map, label_info, output, prefix='val'):
    ...
    def process_item(self, i):
        ...
        output_data = {'img': image, 'idx': idx, 'img_size': (height, width), 'boxes': boxes, 'labels': labels, 'gt_boxes': gt_boxes}      
...
def main():
    ...
    proc = Process(dataset_root, image_size, files, transformer, label_map, label_info, args.output, prefix)
    ...
    for i in tqdm(range(len(files)), desc="Preprocessing dataset", total=len(files)):
        proc.process_item(i)
